module gy3 where

open import lib
not : Bool → Bool
not = λ b → if b then false else true
-- HW
odd : ℕ → Bool
odd = λ x → primrec false (λ _ fx' → (not fx')) x

and : Bool → Bool → Bool
and = λ x y → if x then y else false



pred : ℕ → ℕ
pred = λ n → primrec 0 (λ n' _ → n') n

_+_ : ℕ → ℕ → ℕ
_+_ = λ x y → primrec x (λ y' fy' → suc fy') y

-- HW
-- sum n is the sum of the natural numbers from 0 to n.
-- sum 0 = 0, sum 1 = 1, sum 2 = 3, sum 3 = 6, sum 4 = 10, ...
sum : ℕ → ℕ
sum = λ x → primrec zero (λ x' fx' → fx' + (suc x')) x

-- HW
_*_ : ℕ → ℕ → ℕ
_*_ = λ x y → primrec zero (λ y' fy' → x + fy') y

_^_ : ℕ → ℕ → ℕ
_^_ = λ x y → primrec (suc zero) (λ y' fy' → x * fy') y

squaresum : ℕ → ℕ
squaresum = λ x → primrec zero (λ x' fx' → ((suc x') * (suc x')) + fx') x

-- arithmetic progression
-- a0
-- common difference: d
-- n'th member
-- an = a0 + n*d
ap : ℕ → ℕ → ℕ → ℕ
ap = λ a0 d n → primrec a0 (λ n' fn' → fn' + d) n

isZero : ℕ → Bool
isZero = λ n → primrec true (λ _ _ → false) n

_-_ : ℕ → ℕ → ℕ
_-_ = λ x y → primrec x (λ y' fy' → (pred fy')) y

_eq_ : ℕ → ℕ → Bool
-- _eq_ = λ x y → primrec (if (isZero y) then true else false) (λ y' fy' → ) y
_eq_ = λ x →
  primrec
    isZero
    (λ x' eq' y → primrec false (λ y' fy' → eq' y') y)
    x

_≥_ : ℕ → ℕ → Bool
_≥_ = λ x → 
  primrec
    isZero
    ((λ x' ge' y →
      primrec
        true
        (λ y' fy' → ge' y')
        y))
    x

_>_ : ℕ → ℕ → Bool
-- _>_ = λ x y  → (and ( x ≥ y ) (not (x eq y)))
_>_ = λ x →
  primrec
    (λ y → false)
    (λ x' g' y →
      primrec
        true
        (λ y' _ → g' y')
        y)
        
    x




---------------------------------------------------------
-- Descartes product
---------------------------------------------------------

-- define all the elements of Bool × Bool!

p1 p2 p3 p4 p5 : Bool × Bool
p1 = true , true
p2 = true , false
p3 = false , true
p4 = false , false

p5 = {!!}

uncurryBool : (Bool → Bool → Bool) → Bool × Bool → Bool
uncurryBool = {!!}

swapBool : Bool × Bool → Bool × Bool
-- swap (u , v) = (v , u) for any u, b
swapBool = {!!}

apply : (Bool → Bool) × Bool → Bool
-- apply (f , u) = f u
apply = {!!}



---------------------------------------------------------
-- Abstract types
---------------------------------------------------------

data _≡_ {A : Set}(a : A) : A → Set where
  refl : a ≡ a


idX     : X → X
idX = {!!}

pick    : X → X → X
pick = {!!}

pick'    : X → X → X
pick' = {!!}

f : pick ≡ pick'
f = {!!}

pick*   : X → (X → X) → X
pick* = {!!}

pick?   : (X → X) → X
pick? = {!!}

swap    : X × Y → Y × X
swap = {!!}

curry   : (X × Y → Z) → (X → Y → Z)
curry = {!!}

uncurry : (X → Y → Z) → (X × Y → Z)
uncurry = {!!}

assoc   : (X × Y) × Z → X × (Y × Z)
assoc = {!!}

diag    : X → X × X
diag = {!!}

_∘_     : (Y → Z) → (X → Y) → (X → Z)
_∘_ = {!!}
